...
# "INTERMEDIATE/covar/covar_gcta.txt")
...
for ( i in 1:(nt-1) ){
    ...
    for ( j in ((i+1):nt) ){
        ...
        cmd = rbind(cmd, sprintf("gcta --reml-bivar %s %s --grm %s --covar %s --pheno %s --out %s",
            i, j, grmar, covar, args[5], pathar))

        cmd = rbind(cmd, sprintf("gcta --reml-bivar %s %s --grm %s --covar %s --pheno %s --out %s",
            i, j, grmimp, covar, args[5], pathimp))
