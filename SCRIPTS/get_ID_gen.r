...
if (args[3] == "all") {
    tem = PED[PED$Patient_ID %in% idsnp,"Patient_ID"]
    write.table(cbind(tem, tem),
    args[2],
    ...
    write.table(cbind(tem,tem), args[2], quote = FALSE, row.names = FALSE, col.names = FALSE, sep="\t")
    ...
    tem = PED[(PED$Generation == args[3] & PED$Patient_ID %in% idsnp),"Patient_ID"]
    write.table(cbind(tem,tem),
    args[2],
