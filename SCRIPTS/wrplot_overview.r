...
mu <- tab[, lapply(.SD, function(x) mean(x, na.rm = TRUE)), by = Generation]
longmu <- melt(mu, id.vars = "Generation", variable.name = "pheno", value.name = "mean")

pdf("PLOTS/supdata/pheno_mu.pdf", width = 20, height = 8)
gg1 <- ggplot(longmu[pheno %in% c(paste0("EN",c(1:8,10:13)))], aes(x = pheno, y = mean, color = Generation, group = Generation)) + 
  geom_point(size = 4, position = position_dodge(width = 0.5)) +
  geom_line() +
  scale_color_manual(values = color_gen)

gg2 <- ggplot(longmu[pheno %in% c("EW30","EW40","EW50","EW70")], aes(x = pheno, y = mean, color = Generation, group = Generation)) + 
  geom_point(size = 4, position = position_dodge(width = 0.5)) +
  geom_line() +
  scale_color_manual(values = color_gen)

gg3 <- ggplot(longmu[pheno == "BW32"], aes(x = pheno, y = mean, color = Generation, group = Generation)) + 
  geom_point(size = 4, position = position_dodge(width = 0.5)) +
  geom_line() +
  scale_color_manual(values = color_gen)
